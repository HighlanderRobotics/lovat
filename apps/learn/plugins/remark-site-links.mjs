...
export default function remarkSiteLinks({ websiteUrl = 'https://lovat.app' } = {}) {
  const target = new URL(websiteUrl);
  if (!['http:', 'https:'].includes(target.protocol)) throw new Error('Invalid website URL');
  return (tree) => {
    function visit(node) {
      if (['link', 'definition'].includes(node.type) && node.url) {
        ...
        } catch { /* Relative guide links remain relative. */ }
      }
