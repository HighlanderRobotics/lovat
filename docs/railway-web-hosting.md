...
- Dashboard build variable: LOVAT_API_BASE=https://${{api.RAILWAY_PUBLIC_DOMAIN}}. Docker builds require this explicitly. Native builds retain the existing production default. Redeploy Dashboard when its API URL changes.
- Website: ORIGIN=https://${{RAILWAY_PUBLIC_DOMAIN}}, LOVAT_API_BASE=http://${{api.RAILWAY_PRIVATE_DOMAIN}}:${{api.PORT}}, and LOVAT_SIGNING_KEY=${{api.LOVAT_SIGNING_KEY}} (confirm the API's actual signing variable name). Use the actual configured API port; do not assume one.
